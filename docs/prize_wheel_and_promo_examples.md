...
- **discount_percent**: персональная скидка для пользователя. `prize_value` — строка формата:
  ...
  - опционально: `min_months`/`max_months` в effects ограничивают действие скидки по длительности тарифа.
...
```
- Скидка 20% только на тарифы от 3 до 12 месяцев:
```json
{ "discount_percent": 20, "uses": 1, "min_months": 3, "max_months": 12 }
```
- Скидка 20% навсегда:
```json
