...
const normalizeRange = async (database, table, column, min, max) => {
  const now = new Date();
...
export default function registerEndpoint(router, { database }) {
  router.get("/total-users", async (req, res) => {
    ...
    const { actualStart, maxDate } = await normalizeRange(database, "users", "registration_date", min, max);
  ...
  router.get("/connections", async (req, res) => {
    ...
    const minConn = await getMinDate(database, "connections", "at");
    const actualStart = minConn
      ? new Date(Math.max(minDate.getTime(), minConn.getTime()))
      : minDate;
    ...
    const raw = await database.raw(
      ...
      ORDER BY ds.report_timestamp;
      `,
      [actualStart, maxDate, period, period]
    );
    const results = raw?.rows ?? raw;
    res.json({
      results,
