...
export default function registerHook({ filter }, { database, exceptions }) {
  ...
  const validate = async (payload, keys) => {
    ...
    if (Number.isNaN(newProb) || newProb < 0 || newProb > 1) {
      throw new InvalidPayloadException("Вероятность приза должна быть в диапазоне от 0 до 1");
    }
    ...
    if (newType === "subscription") {
      ...
      if (!Number.isFinite(days) || days <= 0) {
        throw new InvalidPayloadException(
          "Для типа 'subscription' поле 'prize_value' должно быть целым числом дней (> 0)"
        );
      }
    }
    ...
    if (candidateTotal > 1.0 + 1e-9) {
      throw new InvalidPayloadException(
        `Сумма вероятностей активных призов превышает 100%: ${candidateTotal.toFixed(4)}`
      );
    }
