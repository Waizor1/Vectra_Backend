...
import uuid as _uuid_observability

# Observability stack (Sentry + Prometheus + correlation-id).
# All three are feature-flagged via OBSERVABILITY_* env vars (see
# bloobcat/settings.py::ObservabilitySettings). Without DSN/flag they are
# pure no-ops, so this block is safe to ship before observability backends
# exist.
...
# ── Sentry init ─────────────────────────────────────────────────────────────
# Must run BEFORE FastAPI() so the SDK can wrap the ASGI app on construction.
# Feature-flagged: empty OBSERVABILITY_SENTRY_DSN → init is skipped and the
# global hub stays in NonRecordingClient mode (sentry_sdk officially supports
# this as no-op). The before_send filter drops /health and /metrics events so
# Prometheus scrapers and healthchecks never spam the Sentry project.
def _sentry_before_send(event, hint):
    try:
        request = event.get("request") or {}
        url = str(request.get("url") or "")
        path_tail = url.split("?", 1)[0].rsplit("/", 1)[-1]
        if path_tail in {"health", "metrics"}:
            return None
    except Exception:
        # On any introspection error, fall through to send — losing the
        # filter is safer than losing the event.
        pass
    return event


if observability_settings.sentry_dsn:
    sentry_sdk.init(
        dsn=observability_settings.sentry_dsn.get_secret_value(),
        environment=observability_settings.sentry_env,
        release=_build_info["version"],
        traces_sample_rate=observability_settings.sentry_traces_sample_rate,
        integrations=[
            FastApiIntegration(),
            AsyncPGIntegration(),
            HttpxIntegration(),
    ...
    logger.info(
        "Sentry enabled, env={} release={} traces_rate={}",
        observability_settings.sentry_env,
        _build_info["version"],
        observability_settings.sentry_traces_sample_rate,
    )
else:
    logger.info("Sentry disabled (OBSERVABILITY_SENTRY_DSN not set)")
...
async def monitor_slow_requests(request: Request, call_next):
    ...
    Исключает /health и /metrics из мониторинга для предотвращения засорения логов
    (Prometheus scraper стучит каждые 10-30s).
    """
    # Пропускаем мониторинг для healthcheck и Prometheus endpoint
...
# ── Prometheus /metrics ─────────────────────────────────────────────────────
# Must be registered AFTER all include_router() calls so the instrumentator
# picks up every route handler. Feature-flagged via OBSERVABILITY_METRICS_ENABLED.
# Endpoint is excluded from rate_limit_middleware automatically (no path match)
# and from monitor_slow_requests via the skip-set above.
# External access to /metrics is blocked at the Caddy layer (see Caddyfile).
if observability_settings.metrics_enabled:
    ...
    logger.info(
        "Prometheus /metrics endpoint disabled (OBSERVABILITY_METRICS_ENABLED=false)"
    )


# ── Correlation-ID — MUST be the LAST add_middleware() call ─────────────────
# In Starlette, `add_middleware()` does `user_middleware.insert(0, ...)`, so
# the LAST registered middleware becomes the OUTERMOST layer — first to run
# in the request pipeline. We want CorrelationId outermost so request_id is
# set in the ContextVar BEFORE rate_limit, CORS, monitor_slow_requests, or
# Prometheus middleware run; their downstream logs/labels can then include
# the same request_id that the handler sees.
app.add_middleware(
    CorrelationIdMiddleware,
    header_name="X-Request-ID",
    generator=lambda: str(_uuid_observability.uuid4()),
)
