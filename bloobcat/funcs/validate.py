...
async def _apply_referral_attribution_existing_user(
    ...
    current_referred_by = int(getattr(db_user, "referred_by", 0) or 0)
    current_utm = getattr(db_user, "utm", None)
...
async def validate(init_data: str = Depends(oauth2_scheme), request: Request = None) -> Users:
    try:
        ...
        if existing_user and not effective_start_param and existing_user.remnawave_uuid:
