...
async def admin_stat(message: Message, command: CommandObject):
    ...
    if not utm:
        await message.answer(
            "❌ Укажите UTM.\n\nПример: <code>/stat telegram_ads</code>",
            parse_mode="HTML",
        )
        return

    now_dt = datetime.now(MOSCOW_TZ)
    moscow_today = now_dt.date()

    stats = await get_utm_stats(utm, moscow_today=moscow_today)

    now_moscow = now_dt.strftime("%d.%m.%Y %H:%M:%S")
    ...
    stats_message = (
        ...
        f"⚡ Активны сейчас: <b>{stats.direct.active_now}</b>\n"
        ...
        f"⚡ Активны сейчас: <b>{stats.total.active_now}</b>\n"
