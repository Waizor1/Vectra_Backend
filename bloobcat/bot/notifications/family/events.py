...
async def _send_user_notification(
    user_id: int, text: str, button_text: str, button_url: str = "/subscription/family"
) -> None:
    try:
        ...
        logger.error(
            "Bad request while sending family notification to %s: %s", user_id, exc
        )
...
async def notify_family_owner_member_joined(
    owner: "Users", member: "Users", allocated_devices: int, reactivated: bool = False
) -> None:
...
async def notify_family_member_joined(
    member: "Users", owner: "Users", allocated_devices: int, reactivated: bool = False
) -> None:
    lang = get_user_locale(member)
    owner_name = owner.name()
    if lang == "ru":
        lead = (
            "Вы снова подключены к семейной подписке."
            if reactivated
            else "Вы подключены к семейной подписке."
        )
        text = (
            f"✅ {lead}\n\n"
        ...
        lead = (
            "You are back in Vectra Connect family subscription."
            if reactivated
            else "You joined Vectra Connect family subscription."
        )
        text = (
            f"✅ {lead}\n\n"
            f"👤 Owner: {owner_name}\n"
...
async def notify_family_member_limit_updated(
    member: "Users", owner: "Users", allocated_devices: int
) -> None:
    lang = get_user_locale(member)
    owner_name = owner.name()
    if lang == "ru":
        text = (
            "Лимит устройств изменён.\n\n"
            f"👤 Организатор: {owner_name}\n"
...
async def notify_family_member_removed(
    ...
    *,
    ...
    if lang == "ru":
        reason = (
            f"👤 Организатор {owner_name} удалил вас из семьи."
            if removed_by_owner
            else "Вы вышли из семьи."
        )
        text = (
        ...
        button_text = "Подписка"
    else:
        reason = (
            f"👤 Owner {owner_name} removed you from family."
            if removed_by_owner
            else "You left the family."
        )
        text = (
...
async def notify_family_owner_invite_revoked(
    owner: "Users", allocated_devices: int
) -> None:
    lang = get_user_locale(owner)
    if lang == "ru":
        text = (
...
async def notify_family_owner_invites_blocked(
    owner: "Users", blocked_until: datetime, reason: str | None = None
) -> None:
    lang = get_user_locale(owner)
    blocked_until_text = blocked_until.strftime("%d.%m.%Y %H:%M UTC")
    reason_text = (
        f"\nПричина: {reason}"
        if (lang == "ru" and reason)
        else (f"\nReason: {reason}" if reason else "")
    )
    if lang == "ru":
        normalized_reason = reason or "контроль безопасности"
        text = (
            "⚠️ Создание приглашений временно ограничено.\n\n"
            f"Причина: {normalized_reason}.\n\n"
            f"До: {blocked_until_text}"
        )
        button_text = "Семья"
    else:
        text = (
...
async def notify_family_owner_invites_unblocked(owner: "Users") -> None:
    lang = get_user_locale(owner)
    if lang == "ru":
        text = "✅ Ограничение снято\n\nСоздание приглашений доступно."
        button_text = "Создать приглашение"
    else:
        text = "✅ Restriction removed\n\nInvite creation is available again."
