...
async def notify_auto_renewal_failure(user, reason: str = "Неизвестная ошибка", will_retry: bool = True):
    ...
    if lang == 'ru':
        text = (
            f"[!] Привет, {user.full_name}! Не удалось автоматически продлить подписку.\n\n"
        ...
        text = (
            f"[!] Hi {user.full_name}! Auto-renewal failed.\n\n"
    ...
    if user.active_tariff_id:
        ...
        if active_tariff and (active_tariff.lte_gb_total or 0) > 0:
            if lang == 'ru':
                ...
                lte_line = f"\n\nLTE limit refreshed: {int(active_tariff.lte_gb_total)} GB."
    if lang == 'ru':
        parts = [f"[OK] Привет, {user.full_name}! Ваша подписка успешно продлена на {days} дней!"]
        ...
        parts = [f"[OK] Hi {user.full_name}! Your subscription has been renewed for {days} days!"]
        if amount_from_balance > 0:
...
async def notify_payment_canceled_yookassa(user, reason: str | None = None):
    ...
    if lang == "ru":
        text = (
            f"[!] Привет, {user.full_name}! Оплата не была завершена или была отменена."
            f"{reason_line}\n\n"
        ...
        text = (
            f"[!] Hi {user.full_name}! The payment was not completed or was canceled."
            f"{reason_line}\n\n"
...
async def notify_subscription_cancelled_after_failures(user):
    ...
    if lang == 'ru':
        text = (
            "[!] Не удалось продлить вашу подписку.\n\n"
