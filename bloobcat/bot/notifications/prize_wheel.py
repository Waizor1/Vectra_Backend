...
from bloobcat.bot.notifications.admin import get_admin_log_chat_id
...
async def notify_prize_won(
    ...
    try:
        ...
        if requires_admin:
            await notify_admins_about_prize(
                user, prize_name, prize_value, history_id, bot
            )
...
async def notify_logs_channel(user: Users, prize_name: str, prize_value: str, bot: Bot):
    try:
        logs_channel = get_admin_log_chat_id()
        if not logs_channel:
...
async def handle_prize_confirmation(
    ...
    try:
        ...
        if history_entry.is_claimed:
            ...
            logger.info(
                f"Повторное нажатие подтверждения/отмены для history_id={history_id} проигнорировано"
            )
            return
        ...
        if confirmed:
            ...
            try:
                if ptype == "subscription":
                    ...
                    user.prize_wheel_attempts = (
                        int(getattr(user, "prize_wheel_attempts", 0) or 0) + 1
                    )
                    await user.save()
            except Exception as e:
                logger.error(
                    f"Ошибка начисления приза по подтверждению (history_id={history_id}): {e}"
                )
            ...
            try:
                ...
                logger.error(
                    f"Ошибка пометки отклонения (history_id={history_id}): {e}"
                )
            await bot.send_message(
                chat_id=admin_id,
...
async def notify_spin_awarded(user: Users, added_attempts: int, total_attempts: int):
    logger.info(
        "Пользовательские уведомления колеса призов отключены. Пропуск notify_spin_awarded: user=%s added=%s total=%s",
