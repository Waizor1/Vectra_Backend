...
async def notify_trial_revoked_hwid(user):
    ...
    try:
        ...
        logger.info("Отправлено уведомление об отзыве триала (HWID) пользователю %s", user.id)
        ...
        logger.error("Ошибка при отправке уведомления об отзыве триала пользователю %s: %s", user.id, e)
