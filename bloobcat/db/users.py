...
# Управление пользователями делается через Directus (collection `users`).
# FastAdmin admin-class и его save_model/delete_model хуки удалены вместе
# с уходом FastAdmin. RemnaWave-синхронизация при изменении полей теперь
# должна выполняться явно через services / API-эндпоинты, а не через
# хуки админ-формы.
