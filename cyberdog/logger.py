...
def setup_logging():
    ...
    def message_filter(record):
        ...
        if record["name"] == "cyberdog.routes.marzban.catcher" and "process_user" in record["function"]:
    ...
    logger.add(
        os.path.join(log_dir, f"cyberdog_{current_time}.log"),
    ...
    logging.getLogger('cyberdog.routes.marzban.client').setLevel(logging.WARNING)
