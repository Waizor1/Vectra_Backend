...
from cyberdog.bot.bot import get_bot_username
from cyberdog.bot.notifications.admin import on_payment
from cyberdog.bot.notifications.user import (
    on_referral_payment, 
    notify_auto_renewal_success_balance, 
    notify_auto_renewal_failure,
    notify_renewal_success_yookassa
)
from cyberdog.db.tariff import Tariffs
from cyberdog.db.users import Users
from cyberdog.funcs.validate import validate
from cyberdog.settings import yookassa_settings
from cyberdog.db.payments import ProcessedPayments
from cyberdog.logger import get_payment_logger
...
async def pay(tariff_id: int, email: str, user: Users = Depends(validate)):
    ...
    if user_balance >= full_price:
        ...
        payment = Payment.create({
            ...
            "metadata": metadata,
            "capture": True,
            "description": f"Оплата заказа CyberDOG (Тариф: {tariff.name})",
            "save_payment_method": True,
            "receipt": {
                "customer": {"email": email},
                "items": [{
                    "description": f"WEB-Сервис CyberDOG ({tariff.name})",
                    "quantity": "1",
                    "amount": {
                        "value": str(amount_to_pay),
                        "currency": "RUB"
    ...
    try:
        ...
        if user_balance >= full_price:
            ...
            payment = Payment.create({
                ...
                "metadata": metadata,
                "capture": True,
                "description": f"Автопродление подписки CyberDOG ({tariff.name})",
                "receipt": {
                    "customer": {"email": user.email if user.email else "auto@cyberdog.ru"},
                    "items": [{
                        "description": f"Автопродление WEB-Сервиса CyberDOG ({tariff.name})",
                        "quantity": "1",
                        "amount": {
                            "value": str(amount_to_pay),
                            "currency": "RUB"
