...
def main() -> None:
    ...
    # Step 1: Create folder collections for navigation
    group_defs = [
    ...
    for group in group_defs:
        ...
        if resp.status_code == 200:
            ...
            continue
        create_payload = {"collection": collection, "schema": None, "meta": payload_meta}
        session.post(f"{base_url}/collections", headers=headers, json=create_payload, timeout=15).raise_for_status()

    print("Step 1: navigation groups created.")

    # Step 2: Assign collections to groups + icons + notes
    ...
    for collection, meta in collection_meta_updates.items():
        session.patch(f"{base_url}/collections/{collection}", headers=headers, json={"meta": meta}, timeout=15).raise_for_status()

    print("Step 2: collections grouped with icons/notes.")

    # Step 3: Field notes + Russian labels
    field_notes = {
        "users": {
    ...
    print("Step 3: field notes and labels updated.")

    # Step 4: Hide service collections
    service_hidden = ["personal_discounts", "hwid_devices_local", "tvcode", "admin", "aerich"]
    for collection in service_hidden:
        session.patch(
            f"{base_url}/collections/{collection}",
            headers=headers,
            json={"meta": {"hidden": True}},
            timeout=15,
        ).raise_for_status()

    print("Step 4: service collections hidden.")
